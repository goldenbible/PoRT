<?php
	...
	else
	{
		...
		foreach ($messages as $message) 
		{
			...
			$statement = $dbh -> prepare('update messages set when_read = now() where to_user_id = :to_user_id and id = :message_id and when_read is null');
			$result = $statement -> execute($row);

			if ($result) $message = "Message[s] was set as read.";
			else $message = "Whoops. We've got issue with messages marking. Please contact support.";
		}
